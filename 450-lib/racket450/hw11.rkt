...
(provide NaN
         ...
         (rename-out
          [Atom? hw11-Atom?]
          [ErrorResult? hw11-ErrorResult?]
          [Array? hw11-Array?]
          [mk-array+ hw11-mk-array-op]))
...
;; Note: nan? (no caps) is built-in racketfn, where input must be real?, so use different name
...
(struct circular-err ErrorResult [name] #:transparent)
...
(define/contract (hw11-slice a slices)
